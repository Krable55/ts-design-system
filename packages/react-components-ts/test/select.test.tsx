...
import SelectTarget from '../src/select/selectTarget';
...
const ExampleSelect = ({ value: initialValue, name, ...props }: SelectProps) => {
  const [value, setValue] = useState<string | string[] | null | undefined>(initialValue || null);
...
describe('<Select />', () => {
  ...
  describe('with multiselect', () => {
    ...
    it('deselects a selected item on second click + Apply', () => {
      ...
      if (wrapper) {
        const wrapperVal = wrapper?.find('input')?.prop('value')
        if (Array.isArray(wrapperVal)) {
          expect(wrapperVal?.sort()).to.eql(selections.sort())
        };
      }
    });
