...
import { Options } from '../internal/option-menu-list/OptionMenuList';
import { AnchorOrientationType } from '../constants';

const SELECT = 'select' as const;
const MULTISELECT = 'multiselect' as const;
const AUTOCOMPLETE = 'autocomplete' as const;

export type SelectType = typeof SELECT | typeof MULTISELECT | typeof AUTOCOMPLETE;
...
export interface SelectProps {
  ...
  onChange?: (arr?: string | string[] | undefined) => void,
  /**
   ...
   * @ignore
   */
  actionLabel?: string, //eslint-disable-line
...
const isControlled = ({ type, applyImmediately }: SelectProps) =>
  type !== MULTISELECT || applyImmediately;

const getActionLabel = ({ actionLabel, applyImmediately }: SelectProps) =>
...
class Select extends Component<SelectProps, SelectState> {
  ...
  getOptions() {
    ...
    let opts = options?.map(opt => (Array.isArray(opt.value) ? opt.value : opt))
      .flat();
...
(Select as any).defaultProps = defaultProps;
