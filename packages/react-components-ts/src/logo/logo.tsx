...
export const SUPPORTED_LOGOS: (keyof typeof logos)[] = [
  'container-registry',
  'discovery',
  'enterprise',
  'insights',
...
export interface LogoProps extends React.SVGAttributes<SVGSVGElement> {
  /** A string specifying the product name from one of the supported set */
  product?: keyof typeof logos | string;
  /** Logo type (full or bug) */
  type?: 'full' | 'bug';
  /** Boolean "inverted" option for logo display on dark backgrounds */
  inverted?: boolean,
...
const Logo = ({ product, type, inverted, expanded, className, ...rest }: LogoProps) => {
  const svgDef: any = path([product || '', type || ''], logos) || path(['puppet', type || ''], logos);
